...
2. **Steps are idempotent** (or harmless to repeat). Everything is at-least-once: a crash
   ...
   A token that does cancel a checkpoint mid-write (`SetValueAsync`, `ReportProgressAsync`, an
   awaited step's breadcrumb) surfaces as an `OperationCanceledException` whatever the store's
   driver reports, and whether the write committed is settled with one read before the next step
   body runs: the execution carries on from its own write, or — should another writer have moved
   the ledger meanwhile — is abandoned before that body, never after it.
...
references through flow state. The `FlowState` that `AwaitChildFlowAsync` returns is this snapshot
on **every** execution — the first completion reads it back from the memo exactly as a replay
does — so parent logic sees one shape before and after a restart.
...
| `StartAsync`'s **publish fails** | After the start's own retry ladder, `StartAsync` throws **`DurableFlowNotDispatchedException`** and **nothing was persisted**. The exception's `FlowId` carries the id the start would have used — including a generated one — so the retry stays idempotent: if the publish had in fact landed (the ambiguous case is deliberately included), the same id dedupes against the run the job created; a retry with a fresh generated id would start a second, independent run. Supply deterministic ids wherever the caller may retry. (The caller's own `cancellationToken` firing during the publish ends the ladder with an `OperationCanceledException` instead — a cancellation, not a transport outage — which still carries the id, in its message and in `Exception.Data["FlowId"]`: the job may have been published, so retry with that id.) One refusal is deterministic although it arrives the same way: on Kafka, a start whose job cannot fit the producer's `message.max.bytes` minus the dead-letter burial headers is refused at publish, retried by the ladder and surfaced as `DurableFlowNotDispatchedException` — do not retry it; raise the producer's `message.max.bytes` (see [Kafka](transport-semantics.md#kafka)) |
...
reached on a host that is already stopping, the delivery is handed back and redelivered — to a
live replica, or after the restart — instead. Nothing went wrong, and the checkpoints are intact, so
running the undo logic there compensates work that is about to be replayed. Both are raised as
`DurableFlowInterruptedException`, which derives from `OperationCanceledException` — so the usual
filter already excludes it, along with caller-token cancellations, which mean the same thing. (A
timer given a token that host stop cancels too — `IHostApplicationLifetime.ApplicationStopping`
injected into the flow — is still handed over or back, not ended as a plain cancellation.) The
executor does not treat a hand-back as a failed attempt: nothing is checkpointed over the ledger
and no span is marked as an error — observers still get `OnRunAttemptFailedAsync`, since the
attempt ended and any step it reported waiting is no longer parked in this process:
...
without the lease — a recovered one, or one won after the lease was lost — by that write; for one
made by a retried failure's message, by the attempt that first records it, not again by each
redelivery that fails the same way) and again at each doubling. Keep step results small (persist
