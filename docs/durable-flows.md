...
| Process crashes **while awaiting** a remote step | The re-run **re-attaches** to the in-flight wait via the persisted correlation-id breadcrumb — the request is *not* re-sent; progress keeps streaming. (One unavoidable sliver: if the response had already been claimed for delivery at the instant of the crash, nothing can replay it — the re-attached wait then ends at the step timeout and the idempotent step restarts fresh) |
