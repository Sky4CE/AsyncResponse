...
| `RecoveryStateExpiry` | Redis, NATS, PostgreSQL, SQL Server, MongoDB | 7 days | How long durable recovery state survives. Also the default wait timeout backstop. Don't set below your longest flow duration. Capped at ~49.7 days — the .NET timer ceiling — because it doubles as the waiter-timeout fallback (the same cap applies to `DefaultTimeout` and `DisposalDrainTimeout`; validation rejects longer values at startup, and a waiter created with a longer explicit timeout is rejected before it registers anything). |
