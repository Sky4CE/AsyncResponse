...
/// <summary>Options for the PostgreSQL AsyncResponse transport.</summary>
public sealed class PostgreSqlAsyncResponseTransportOptions
{
    /// <summary>The transport name reported to reply targets and the startup validator.</summary>
    public const string TransportName = "PostgreSQL";

    /// <summary>Database schema that contains the transport table. Default: <c>public</c>.</summary>
    ...
    /// <summary>PostgreSQL notification channel used to wake subscribers after publishes.</summary>
    public string NotificationChannel { get; set; } = "asyncresponse_transport_notify";

    /// <summary>Creates the schema, table, and indexes on first use. Disable when migrations own DDL.</summary>
    public bool AutoCreateSchema { get; set; } = true;

    /// <summary>Logical queue name used by <see cref="PostgreSqlWorkerTransport"/>.</summary>
    public string WorkerQueue { get; set; } = "worker";

    /// <summary>Worker queue handling options.</summary>
    public PostgreSqlSubscriberOptions WorkerSubscriber { get; } = new();
    ...
    public PostgreSqlSubscriberOptions ResponseSubscriber { get; } = new();
    ...
    /// </summary>
    public Dictionary<string, PostgreSqlReplyTargetOptions> ReplyTargets { get; } = new(StringComparer.Ordinal);
    ...
    /// <summary>Maximum attempts for PostgreSQL publish commands. Set to 1 to disable publish retries.</summary>
    ...
    /// <summary>
    /// The hosting shutdown budget that must contain PostgreSQL subscriber shutdown
    /// (<see cref="ShutdownTimeout"/>) plus <see cref="PostgreSqlSubscriberOptions.BackgroundDrainTimeout"/>
    /// when a subscriber uses <see cref="PostgreSqlAckMode.AckAfterEnqueue"/> — the sum of both
    /// subscribers' shutdown and <see cref="PostgreSqlSubscriberOptions.BackgroundDrainTimeout"/> when
    /// both use it, since the host stops them one after the other. Defaults to the Generic Host
    /// default of 30 seconds. Set to <c>null</c> only when this budget is validated externally.
    /// </summary>
    ...
    public PostgreSqlAsyncResponseTransportOptions AddReplyTarget(string name, string responseQueue)
    {
        ...
        ReplyTargets[name] = new PostgreSqlReplyTargetOptions { ResponseQueue = responseQueue };
        return this;
    }
}

/// <summary>Options for one named PostgreSQL async-response reply target.</summary>
public sealed class PostgreSqlReplyTargetOptions
{
...
/// <summary>Controls when a PostgreSQL transport row is acknowledged relative to handling.</summary>
public enum PostgreSqlAckMode
{
    /// <summary>
    /// Delete the row only after the AsyncResponse handler completes successfully. Handler failures
    /// reschedule the row until <see cref="PostgreSqlSubscriberOptions.MaxDeliveryAttempts"/> is reached.
    /// </summary>
...
public sealed class PostgreSqlBackgroundFailureContext
{
    internal PostgreSqlBackgroundFailureContext(string queue, string subscriberRole, int attempt, string? correlationId, Exception exception)
    {
        Queue = queue;
...
/// <summary>Per-queue PostgreSQL subscriber behavior.</summary>
public sealed class PostgreSqlSubscriberOptions
{
