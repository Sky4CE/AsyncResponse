...
public sealed class NatsWorkerTransport : IWorkerTransport
{
    ...
    private async Task EnsureWorkerStreamOnceAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            if (!_streamEnsured)
            {
                // Retried on the SAME terms as the publish below: provisioning is a JetStream API
                // request like any other, and it runs when that request is likeliest to time out —
                // the first publish after startup, or after any hiccup, since the flag only
                // latches on success.
