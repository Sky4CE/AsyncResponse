        ...
        public void Schedule()
            => _ = Task.Run(async () =>
            {
                ...
                catch (Exception ex)
                {
                    ...
                    owner._logger.LogError(ex, "Error handling MongoDB waiter timeout for correlationId {CorrelationId}.", correlationId);
                }
            });
    }
        ...
        public async ValueTask CleanupOnceAsync(bool deleteRecoveryState)
        {
            ...
            try
            {
                ...
                catch (Exception ex)
                {
                    ...
                    _owner._logger.LogError(ex, "Failed to delete MongoDB recovery state for correlationId {CorrelationId}.", _correlationId);
                }
                ...
                {
                    // Best-effort: an orphaned subscriber document ages out via the heartbeat timeout.
                    _owner._logger.LogError(ex, "Failed to delete MongoDB subscriber document for correlationId {CorrelationId}.", _correlationId);
                }
            }
            finally
            {
