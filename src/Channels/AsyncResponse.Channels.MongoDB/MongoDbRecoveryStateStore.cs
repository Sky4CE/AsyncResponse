    ...
    ILogger<MongoDbRecoveryStateStore> _logger) : IRecoveryStateStore, IRecoveryStateScanner
{
    ...
    public Task<bool> TryDeleteAsync(string correlationId, CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(correlationId);
        cancellationToken.ThrowIfCancellationRequested();
        return _store.DeleteRecoveryStateAsync(correlationId, registrationId: null, cancellationToken);
    }
    ...
    private RecoveryState? DeserializeState(string json, string? correlationId)
    {
        try
        {
            ...
            {
                _logger.LogWarning(
                    "MongoDB recovery state for correlationId {CorrelationId} has schema version {SchemaVersion}, newer than this build supports ({Current}); rejecting it instead of risking a misinterpreted recovery.",
