    ...
    ILogger<PostgreSqlRecoveryStateStore> _logger) : IRecoveryStateStore, IRecoveryStateScanner
{
    ...
    private RecoveryState? DeserializeState(string json, string? correlationId)
    {
        try
        {
            ...
            {
                _logger.LogWarning(
                    "PostgreSQL recovery state for correlationId {CorrelationId} has schema version {SchemaVersion}, newer than this build supports ({Current}); rejecting it instead of risking a misinterpreted recovery.",
