...
public sealed class SqlServerFlowStateStore : IFlowStateStore
{
    ...
    public async Task<FlowLeaseObservation?> ObserveLeaseAsync(string flowId, CancellationToken cancellationToken = default)
    {
        ...
        // The two lease columns exactly as stored — deliberately no SYSUTCDATETIME() predicate,
        ...
        // database clock. datetime2 carries no zone and reads back Unspecified; the value is
        // SYSUTCDATETIME() arithmetic, and the shared shaper stamps it UTC at full datetime2(7)
        // precision.
        await using var connection = await OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
        await using var command = connection.CreateCommand();
